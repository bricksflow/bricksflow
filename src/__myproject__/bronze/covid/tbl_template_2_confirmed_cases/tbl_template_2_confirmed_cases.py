...
# MAGIC You might have noticed some variables which were used in the functions above like `source_csv_path` and `output_table_name`. This variables/parameters are gathered from config file that is editable only from Git/Local. It is not possible to change config within Databricks (yet).
# MAGIC
...
# MAGIC `schema = table_manager.getSchema('bronze_covid.tbl_template_2_confirmed_cases')`
# MAGIC
# MAGIC
...
def save_table_ronze_covid_tbl_template_2_confirmed_cases(df: DataFrame, logger: Logger, table_manager: TableManager):
    ...
    # table_manager.delete('bronze_covid.tbl_template_2_confirmed_cases')

    if table_manager.exists("bronze_covid.tbl_template_2_confirmed_cases"):
        logger.info(f"Table {output_table_name} exists. Appending...")
    else:
        table_manager.create("bronze_covid.tbl_template_2_confirmed_cases")

    # or you can use TableManager.recreate()
    # table_manager.recreate("bronze_covid.tbl_template_2_confirmed_cases")
